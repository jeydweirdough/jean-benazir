        ...
        <div className="lightbox" onClick={() => setSelectedProject(null)}>
          <div className="lightbox-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="lb-modal-header">
              <span className="lb-modal-header-title">Project Details</span>
              <button className="lb-modal-close" onClick={() => setSelectedProject(null)}>&#x2715;</button>
            </div>

            {/* Single scrollable two-column body */}
            <div className="lb-two-col-body">

              {/* LEFT */}
              <div className="lb-left">
                <h3 className="lb-left-title">{selectedProject.title}</h3>
                <div className="lb-mini-specs">
                  <div>
                    <span className="lb-mini-label">Year</span>
                    <span className="lb-mini-val">{selectedProject.year}</span>
                  </div>
                  <div>
                    <span className="lb-mini-label">Location</span>
                    <span className="lb-mini-val">{selectedProject.location}</span>
                  </div>
                </div>
                <div className="lb-img-card">
                  <img src={selectedProject.image} alt={selectedProject.title} />
                </div>
                <div className="lb-tab-bar">
                  <span className="lb-tab">Scope of Work</span>
                </div>
                <div className="lb-scope-body">
                  {selectedProject.scope.map((item, index) => (
                    <div key={index}>
                      <p className="lb-scope-item-label">Item {index + 1}</p>
                      <p className="lb-scope-item-text">{item}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="lb-right">
                <h4 className="lb-right-title">Project Details</h4>
                <p className="lb-right-subtitle">Engineering scope and technical parameters for this project.</p>

                <div className="lb-form-field">
                  <label className="lb-form-label">Role</label>
                  <span className="lb-chip-display">{selectedProject.role}</span>
                </div>

                <div className="lb-form-field">
                  <label className="lb-form-label">Client Type</label>
                  <span className="lb-input-display">{selectedProject.client}</span>
                </div>

                <div className="lb-form-field">
                  <label className="lb-form-label">Description</label>
                  <span className="lb-textarea-display">{selectedProject.desc}</span>
                </div>

                <div className="lb-form-field">
                  <label className="lb-form-label">Technical Specifications</label>
                  <div className="lb-specs-table">
                    <div className="lb-spec-row">
                      <span>Capacity / Sizing</span>
                      <span>{selectedProject.capacity_yield}</span>
                    </div>
                    <div className="lb-spec-row">
                      <span>Inverters / Auxiliaries</span>
                      <span>{selectedProject.inverter_rating}</span>
                    </div>
                    <div className="lb-spec-row">
                      <span>Design &amp; Drawings</span>
                      <span>{selectedProject.design_drawings}</span>
                    </div>
                    <div className="lb-spec-row">
                      <span>Standards Compliance</span>
                      <span>{selectedProject.standards_code}</span>
                    </div>
                  </div>
                </div>

                <div className="lb-form-field">
                  <label className="lb-form-label">Result &amp; Impact</label>
                  <span className="lb-result-display">"{selectedProject.result}"</span>
                </div>

                <button className="lb-cta-btn" onClick={() => setSelectedProject(null)}>
                  Close Details
                </button>
              </div>
